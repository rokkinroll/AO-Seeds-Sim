...
P_jet = 0.1 * (Mdot_crit_val * c**2) / seq_vals  # erg/s (eta=0.1 radiative)
eps = k_eps * (n_s / H0_val**3) * seq_vals  # Scaled to ~10^{-3}

# Monte Carlo for uncertainty (10^3 samples, n_s ±20%)
np.random.seed(42)
n_s_samples = n_s + np.random.normal(0, 0.2 * n_s, 1000)
eps_samples = k_eps * (n_s_samples[:, np.newaxis] / H0_val**3) * seq_vals[np.newaxis, :]
sigma_eps = np.std(eps_samples, axis=0)  # Propagation approx
...
ax1.set_xlabel('SEQ'); ax1.set_ylabel('P_jet (erg s$^{-1}$)'); ax1.grid(alpha=0.3); ax1.legend()
...
ax2.set_xlabel('SEQ'); ax2.set_ylabel('ε (dimensionless)'); ax2.grid(alpha=0.3); ax2.legend()
